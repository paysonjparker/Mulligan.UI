    ...
    this.subscriptions = this.golfCourseService.getGolfCourseById(this.activatedRoute.snapshot.paramMap.get("Id")!).subscribe({
      next: (data) => {
        ...
        this.editGolfCourseForm = this.formBuilder.group({
          ...
          city: new FormControl<string>(this.golfCourse.city, { validators: [Validators.required] }),
          subdivision: new FormControl<string>(this.golfCourse.subdivision ?? '', { validators: [Validators.required] }),
          country: new FormControl<string>(this.golfCourse.country, { validators: [Validators.required] }),
